...
//oModel:AddCalc( 'COMP022CALC1', 'ZA1MASTER', 'ZA2DETAIL', 'ZA2_AUTOR', 'ZA2__TOT01', , { | oFW | COMP022CAL( oFW, .T. ) },,'Total Pares' )
//oModel:AddCalc( 'COMP022CALC1', 'ZA1MASTER', 'ZA2DETAIL', 'ZA2_AUTOR', 'ZA2__TOT02', 'COUNT', { | oFW | COMP022CAL( oFW, .F. ) },,'Total Impares' )


//-------------------------------------------|
//inclusão fernando
//Bloco para valiadar a condição do AddCalc()|
//-------------------------------------------|

//oModel:AddCalc( 'COMP022CALC1', 'ZA1MASTER', 'ZA2DETAIL', 'ZA2_AUTOR', 'ZA2__TOT01', 'SUM', bCond,,'Qtde total' )
oModel:AddCalc( 'COMP022CALC1', 'ZA1MASTER', 'ZA2DETAIL', 'ZA2_ITEM', 'ZA2__TOT02', 'SUM',/* { | oFW | u_ValCond( .F. ) }*/,,'Total' )
...
User Function ValCond( lPar )
Local lRet := .T.
Local oModel      := FWModelActive()
Local oModelFilho := oModel:GetModel( "ZA2DETAIL")
Local nTotal := 0
 // validação do contador
For nI := 1 To oModelFilho:Length()
	oModelFilho:GoLine( nI )
	nTotal += Val(oModel:GetValue( 'ZA2DETAIL', 'ZA2_ITEM' ))
	If  nTotal > 5 // se o item for maior que dois, não inclue no totalizador
		lRet := .F.
	Endif
Next	
